...
def conv1x1(input_, output_dim,
            init=tf.contrib.layers.xavier_initializer(), resue=False, name='conv1x1'):
  k_h = 1
  k_w = 1
  d_h = 1
  d_w = 1
  with tf.variable_scope(resue+name):
    w = tf.get_variable(
...
def sn_conv1x1(input_, output_dim, update_collection,
              init=tf.contrib.layers.xavier_initializer(), resue=False, name='sn_conv1x1'):
  with tf.variable_scope(resue+name):
    k_h = 1
    k_w = 1
    d_h = 1
...
def sn_non_local_block_sim(x, update_collection, name, reuse=False, init=tf.contrib.layers.xavier_initializer()):
  with tf.variable_scope(name):
    batch_size, h, w, num_channels = x.get_shape().as_list()
    location_num = h * w
    downsampled_num = location_num // 4
    # theta path
    theta = sn_conv1x1(x, num_channels // 8, update_collection, init, reuse, 'sn_conv_theta')
    theta = tf.reshape(
        theta, [batch_size, location_num, num_channels // 8])

    # phi path
    phi = sn_conv1x1(x, num_channels // 8, update_collection, init, reuse, 'sn_conv_phi')
    ...
    # g path
    g = sn_conv1x1(x, num_channels // 2, update_collection, init, reuse, 'sn_conv_g')
    ...
    attn_g = sn_conv1x1(attn_g, num_channels, update_collection, init, reuse, 'sn_conv_attn')
